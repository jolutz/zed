...
- **Copy Share Link** â€” copies a `zed://skill` link that embeds the skill, ready to send to someone else (see [Sharing Skills](#sharing-skills))
- **Open** â€” opens the skill's `SKILL.md` file in the editor
- **Delete** â€” removes the skill folder from disk
...
You can hand a skill to a teammate without hosting it anywhere. In the Skills settings page, click the **link** icon on a skill row to copy a `zed://skill?data=â€¦` link to your clipboard.
...
â”œâ”€â”€ scripts/          # Optional: scripts the agent can run
â”œâ”€â”€ references/       # Optional: additional documentation
â””â”€â”€ assets/           # Optional: templates and static files
```
