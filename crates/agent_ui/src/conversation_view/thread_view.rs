...
pub(crate) enum PermissionSelection {
    ...
    Choice(usize),
    /// "Select optionsâ€¦" mode where individual command patterns can be toggled.
...
impl ThreadView {
    pub(crate) fn new(
        ...
        // local list rather than appending â€” this also clears stale issues
            ...
            .downcast::<agent::NativeAgentConnection>()
        {
            ...
            subscriptions.push(cx.subscribe(
                ...
                move |this: &mut Self, _agent, event: &SkillLoadingIssuesUpdated, cx| {
                    ...
                    // list â€” the underlying file must have been fixed or removed, so a
    ...
    pub fn handle_toggle_command_pattern(
        ...
        match self.permission_selections.get_mut(&tool_call_id) {
            Some(PermissionSelection::SelectedPatterns(checked)) => {
                // Already in pattern mode â€” toggle the individual pattern.
    ...
    fn render_context_compaction(
        ...
        let header_label = match compaction.status {
            acp_thread::ContextCompactionStatus::InProgress => "Compacting Contextâ€¦",
    ...
    fn render_entry(
        ...
        let primary = match &entry {
            ...
            AgentThreadEntry::ToolCall(tool_call) => {
                ...
                // renders as a useless "Canceled" card â€” hide those entirely.
...
;
            ...
            .menu(move |window, cx| {
                ...
                Some(ContextMenu::build_persistent(
                    ...
                    move |menu, _window, cx| {
                        ...
                        menu = menu.separator().header("Select Optionsâ€¦");
                                    ...
                                    view.update(cx, |this, cx| {
                                        ...
                                        match selection {
                                            Some(PermissionSelection::SelectedPatterns(_)) => {
                                                // Already in pattern mode â€” toggle.
    ...
    fn render_image_output(
        ...
        let dimensions_label = if let Some(size) = dimensions {
            format!("{}Ã—{} {}", size.width, size.height, format_name)
        } else {
            format_name.into()
        };
...
/// Returns the name of the leading built-in (native-category) slash command â€”
/// e.g. `compact` for `/compact` or `/compact summarize the API work` â€” whether
...
mod tests {
    ...
    async fn test_open_link_bare_path(cx: &mut gpui::TestAppContext) {
        ...
        // Relative path â€” call from multi_workspace so the inner workspace entity is not locked
