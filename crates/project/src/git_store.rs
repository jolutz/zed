...
pub async fn resolve_git_worktree_to_main_repo(fs: &dyn Fs, path: &Path) -> Option<PathBuf> {
    ...
    // It's a .git file â€” parse the gitdir: pointer
...
pub fn worktrees_directory_for_repo(
    ...
    // `Path::is_absolute()` requires a drive letter â€” so `/tmp/worktrees`
...
///   returned â€” it is already a meaningful on-disk path.
